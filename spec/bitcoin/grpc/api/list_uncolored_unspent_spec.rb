...
RSpec.describe Bitcoin::Grpc::Api::ListUncoloredUnspent do
  describe '#execute' do
    subject { described_class.new(spv).execute(request) }

    let(:request) do
      Bitcoin::Grpc::ListUncoloredUnspentRequest.new(
    ...
    it { expect(subject.utxos.size).to eq 1 }
    it { expect(subject.utxos[0].value).to eq 6 }
  end
end
