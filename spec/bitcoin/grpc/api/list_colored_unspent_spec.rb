...
RSpec.describe Bitcoin::Grpc::Api::ListColoredUnspent do
  describe '#execute' do
    subject { described_class.new(spv).execute(request) }

    let(:request) do
      Bitcoin::Grpc::ListColoredUnspentRequest.new(
    ...
    it { expect(subject.assets.size).to eq 2 }
    it { expect(subject.assets[0].value).to eq 3 }
  end
end
