...
RSpec.describe Bitcoin::Grpc::Api::GetBalance do
  describe '#execute' do
    subject { described_class.new(spv).execute(request) }

    let(:request) { Bitcoin::Grpc::GetBalanceRequest.new(account_name: account_name) }
    ...
    it { expect(subject.balance).to eq 8 }
  end
end
