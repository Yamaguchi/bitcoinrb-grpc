  ...
  module Grpc
    class Server < Bitcoin::Grpc::Blockchain::Service
      ...
      def watch_tx_confirmed(request, call)
        ...
        Receiver.spawn(:receiver, channel, publisher, [EventTxConfirmed])
        ResponseEnum.new(request, channel, WatchTxConfirmed).each
      end

      def watch_utxo(request, call)
        watcher << request
        channel = Concurrent::Channel.new
        Receiver.spawn(:receiver, channel, publisher, [EventUtxoRegistered, EventUtxoSpent])
        ResponseEnum.new(request, channel, WatchUtxoResponse).each
      end

      def watch_token(request, call)
        watcher << request
        channel = Concurrent::Channel.new
        Receiver.spawn(:receiver, channel, publisher, [EventTokenIssued, EventTokenTransfered])
        ResponseEnum.new(request, channel, WatchTokenResponse).each
      end
    end
